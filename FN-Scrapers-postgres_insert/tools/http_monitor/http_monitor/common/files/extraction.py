...
def remote_extract(name, fil, column_spec=None, update_flag=None, **kwargs):
    """
    files.File object to remote extraction entities list via docservice.
    """

    session = Session.get()
    # TODO: Review column_spec values (ints? ttypes?)
    try:
        return session.docserv_client.extract_content(
            fil.download_id, name, column_spec=column_spec,
