...
class DocumentItem(JsonSchemaItem):
    ...
    schema_file_path = schema_dir + os.sep + 'peru.json'
...
class PERUBill(DocumentItem):
    """
    Object representing an Peru Bill document scraped from Peru bills website.
    """
    schema_dir = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'schemas'
    schema_file_path = schema_dir + os.sep + 'peru.json'
    with open(schema_file_path) as fp:
    ...
    def add_legislature(self, legislature):
        self[u'legislature'] = legislature

    def add_introduction_date(self, introduction_date):
        self[u'introduction_date'] = introduction_date

    def add_proponent(self, proponent):
        self[u'proponent'] = proponent

    def add_parliamentary_group(self, parliamentary_group):
        self[u'parliamentary_group'] = parliamentary_group
    ...
    def add_action_by_obj(self, action):
        if u'actions' not in self:
            self[u'actions'] = []
            if isinstance(action, DocumentActions):
                self[u'actions'].append(action.to_json())
            else:
                raise ValueError("DocumentAction class argument is required")
        else:
            if self[u'actions'] and isinstance(self[u'actions'], list):
                if isinstance(action, DocumentActions):
                    self[u'actions'].append(action.to_json())
                    self[u'actions'] = [dict(tupleized) for tupleized in
                                       set(tuple(item.items()) for item in self[u'actions'])]
                else:
                    raise ValueError("DocumentAction class argument is required")
