...
class DocumentItem(JsonSchemaItem):
    ...
    schema_file_path = schema_dir + os.sep + 'france.json'
...
class FRANCEBill(DocumentItem):
    """
    Object representing an FRANCE Bill document scraped from FRANCE bills website.
    """
    schema_dir = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'schemas'
    schema_file_path = schema_dir + os.sep + 'france.json'
    with open(schema_file_path) as fp:
    ...
    def add_stages_by_obj(self, stage):
        if u'stages' not in self:
            self[u'stages'] = []
            if isinstance(stage, stages):
                self[u'stages'].append(stage.to_json())
            else:
                raise ValueError("stages class argument is required")
        else:
            if self[u'stages'] and isinstance(self[u'stages'], list):
                if isinstance(stage, stages):
                    self[u'stages'].append(stage.to_json())
                    self[u'stages'] = [dict(tupleized) for tupleized in set(tuple(item.items()) for item in self[u'stages'])]
                else:
                    raise ValueError("stages class argument is required")

    def add_chamber_of_origin(self, chamber_of_origin):
        self[u'chamber_of_origin'] = chamber_of_origin

    def add_description(self, description):
        self[u'description'] = description

    def add_other_chamber_source_url(self, other_chamber_source_url):
        self[u'other_chamber_source_url'] = other_chamber_source_url

    def add_scrape_source(self, scrape_source):
        self[u'scrape_source'] = scrape_source

    def add_long_title(self, long_title):
        self[u'long_title'] = long_title
